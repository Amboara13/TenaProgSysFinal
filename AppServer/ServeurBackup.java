...
public class ServeurBackup {
    ...
    public static void main(String[] args) {
        ...
        int port = Conf.getPortBackup();
        String filenamesauvegarde = Conf.getFilenamesauvegarde();

        // Si le port est 0, cela signifie qu'il n'a pas été correctement défini
        if (port == 0) {
    ...
    private static void handleClient(Socket socketClient) {
        try (
            ...
            out.println("Bienvenue " + username + ", vous êtes connecté au serveur de sauvegarde.");
    ...
    public static void recupMessClient(Socket socketClient, BufferedReader in, PrintWriter out) {
        try {
            ...
            while ((message = in.readLine()) != null) {
                ...
                try {
                    String mesrtr = Relation.requeteLecture(message);
